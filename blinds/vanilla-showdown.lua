...
SMODS.Blind {
    ...
    loc_vars = function(self)
        return { vars = hypb_ethos_locs[hypb_evens + 1] } 
    end,
    collection_loc_vars = function(self)
    ...
    calculate = function (self, card, context)
        ...
        if context.setting_blind then
            ...
            hypb_ethos_debuff_rank = pseudorandom_element(hypb_ethos_ranks, pseudoseed('ETHOS'))
        end
        if context.hand_drawn then
            for i, v in pairs(context.hand_drawn) do
                if v:get_id() == hypb_ethos_debuff_rank then
                    v:set_debuff(true)
                    v:juice_up()
                end
            end
        ...
        if context.press_play then
            for i, rank in ipairs(hypb_ethos_ranks) do
                 if rank == hypb_ethos_debuff_rank then
                    table.remove(hypb_ethos_ranks, i)
                 end
            end
            hypb_ethos_perma[hypb_ethos_debuff_rank] = true
            hypb_ethos_debuff_rank = pseudorandom_element(hypb_ethos_ranks, pseudoseed('ETHOS'))
            if G.jokers.cards[1] then
                local jokers = {}
                for i = 1, #G.jokers.cards do
    ...
    recalc_debuff = function(self, card)
        if card:get_id() == hypb_ethos_debuff_rank then
            return true
        end
